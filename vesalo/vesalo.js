...
't', 'u', 'v', 'z', 'ž'];
var rec, reci, slova, slovo, pok, crtica, polja, pokusaj=10, sme_slovo=[], brojac=0, ima_slovo, nepogodjena_rec, broj_pogodjenih = 0;
...
for(let i=0; i < slova.length; i++){
    ...
    slova[i].onclick=function(){
        ...
        if (rec.indexOf(slovo) >= 0){
            ...
            if(pokusaj > 0 && broj_pogodjenih !== rec.length && ima_slovo == false){
                pokusaj--;
                brojac++;
